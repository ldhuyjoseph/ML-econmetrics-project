...
Split the data set into two pieces — a training set and a testing set. This consists of random sampling without replacement a portion (try with 70 percent) of the rows (you can vary this) and putting them into your training set. The remaining data set is your testing set.
...
```{r}
tree <- rpart(formula = Sale_Price ~ .,#Gr_Liv_Area + Overall_Qual,
              data    = ames_train,
              cp=0,
              #cp=0.01,
              method="anova")  # use method="class" for classification
...
```{r, echo=T, warning=F, message=F}
pred <- predict(pruned.tree, newdata = ames_test)
...
```{r, echo=T, warning=F, message=F}
y.hat <- predict(pruned.tree, newdata = ames_train)
...
add_col <- dummy_cols(ames_train$Overall_Qual, remove_selected_columns = T) # transform qualitative var in a set of dummy variables
descriptive <- cbind(ames_train[,c("Sale_Price", "Gr_Liv_Area")], add_col, leaf)
...
```{r, echo=T, warning=F, message=F}
vip(pruned.tree, target = "Sale_Price", metric = "rmse")
```
...
gbm.fit <- gbm(
  formula = Sale_Price ~ ., 
  distribution = "gaussian", # guassian is used for continuous variables
  data = ames_train,
  n.trees = 10000,
  n.minobsinnode = 5,
  interaction.depth = 1,
...
```{r, echo=T, warning=F, message=F}
pred <- predict(gbm.fit, newdata = ames_test)
RMSE(pred = pred, obs = ames_test$Sale_Price)

##for Carseats
pred <- predict(gbm.fit, newdata = Carseats_test)
...
```{r, echo=T, warning=F, message=F}
pred <- predict(gbm.fit2, newdata = Carseats_test)
RMSE(pred = pred, obs = Carseats_test$Sales)
```
...
```{r, echo=T, warning=F, message=F}
# ADD YOUR CODE HERE
vip(gbm.fit2, target = "Sale_Price", metric = "rmse")
...
pred <- predict(pruned.tree, newdata = Carseats_test)
RMSE(pred = pred, obs = Carseats_test$Sales)
```
