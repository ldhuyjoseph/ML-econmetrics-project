...
#back to the question of wheher to include other ys
n <- nrow(subset_belief)
split <- sample(1:n, floor(n/2))
fold1 <- subset_belief[split, ]
fold2 <- subset_belief[-split, ]
...
```{r, warning=F, message=F, echo=T, fig.align='center'}
set.seed(123)
causal_tree <- honest.causalTree(y_belief_report_num ~ ., # formula
                          data = fold1,  # data for splitting
                          treatment = fold1$d_treatment_source, # treatment variable
                          est_data = fold2,   # data for computing CATE
                          est_treatment = fold2$d_treatment_source, # treatment variable
...
honest_ols <- lm( y_belief_report_num ~ leaf + d_treatment_source * leaf - d_treatment_source -1, data =  fold2)
summary(honest_ols)
...
```
##to copy
to interprete the results, how about the coeff of interaction term
```{r, warning=F, message=F, echo=T, fig.align='center'}
honest_ols <- lm( y_belief_report_num ~ leaf + d_treatment_source * leaf - d_treatment_source -1, data =  fold2)
summary(honest_ols)
```
